...
with tab_market:
    ...
    with col_dl1:
        ...
        btn_download = st.button("正在分段下载", type="primary", use_container_width=True)
    ...
    if cached_all_files:
        st.subheader("📈 交互式行情看板 (底部拖拽与框选时间)")
...
# ==========================================
with tab_strategy:
    st.header("🎛️ 股票数据条件筛选")
    st.markdown("💡 **操作指南**：初始状态下工具**不实施任何拦截**。展开你需要筛选的条件，勾选激活，然后执行筛选。")
    
    cached_files = [f for f in os.listdir(STORE_DIR) if f.endswith("_daily.csv")]
    if not cached_files:
        st.warning("⚠️ 数据为空，请先前往第一页下载股票数据。")
    else:
        ...
        with col_f1:
            with st.expander("💸 维换手量特征过滤", expanded=False):
                ...
                vol_count = st.number_input("上述情况至少发生次数：", min_value=1, value=1)
                
            with st.expander("📈 均线与趋势形态", expanded=False):
                enable_ma = st.checkbox("激活：日线均线排列筛选")
                ma_strategy = st.radio("均线规则", ["收盘价站上 MA20 (短线转强)", "MA5 > MA10 > MA20 (标准多头排列)"])
                
        with col_f2:
            with st.expander("⏱️ 60分钟 MACD 过滤", expanded=False):
                enable_macd = st.checkbox("激活：60分钟 MACD 过滤")
                macd_mode = st.radio("副图 MACD 形态要求", ["DIF与DEA均在零轴上方 (水上强势)", "DIF 刚上穿 DEA (水下金叉/水上金叉)"])

            with st.expander("🛑 极端涨跌筛选", expanded=False):
                enable_range = st.checkbox("激活：极端涨跌筛选")
                min_gain = st.number_input("全期间内最高涨幅不低于(%)", value=10.0)
                max_drawdown = st.number_input("最高点产生后的最大回撤不超过(%)", value=-15.0)

        if st.button("🚀 组合选股启动 (从已下载的股票数据中筛选)", type="primary"):
            passed_stocks = []
...
with tab_ai:
    ...
    st.header("🤖 AI 智能语义选股")
    st.markdown("💡 **操作指南**：直接使用自然语言描述选股条件，AI 会自动为你生成参数并直接调用本地筛选条件库。")
    ...
    # ---- 3. 双驱动交互引擎 UI 切换 ----
    engine_mode = st.radio("⚙️ AI渠道设置", ["🤖 API 直连自动编译", "📋 大模型提示词 (免 API Key 复制粘贴)"], horizontal=True)
    ...
    if engine_mode == "🤖 API 直连全自动编译":
        ...
        if st.button("🧠 进行 AI 自动编译", type="primary", use_container_width=True):
            if not api_key:
    ...
    if parsed_json_str:
        ...
        try:
            ...
            if not filters:
                ...
                cached_files = [f for f in os.listdir(STORE_DIR) if f.endswith("_daily.csv")]
                if not cached_files:
                    st.error("⚠️ 本地无数据文件，请先前往第一页批量下载。")
                else:
                    ai_passed_stocks = []
